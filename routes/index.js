...
router.get("/upload/images/:filename", async (req, res) => {
  try {
    const response = await getFromR2("upload/images/" + req.params.filename);
...
router.get("/item/:id", async (req, res) => {
  try {
    ...
    const category = categories.find(
      (c) => c._id.toString() === item.category.toString(),
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        imagePath = await uploadToR2(req.file);
  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        updateData.image = await uploadToR2(req.file);
        await deleteFromR2(oldItem.image);
      }
